...
2. Check the **Portability** column:
   - **[D] Drop-in** -- Copy the file as-is; no repo-specific changes needed.
   - **[C] Configurable** -- Works out of the box but you'll want to tweak values (labels, days, team names, etc.).
   - **[S] Needs Setup** -- Requires repo-specific secrets, tokens, project IDs, or external service configuration.
...
|---|---------|------|-------------|:-----------:|
| 1 | [Auto-Label Issues by Keywords](#1-auto-label-issues-by-keywords) | `auto-label-issues.yml` | Scans issue titles and bodies for keywords and applies labels automatically (e.g., `bug`, `enhancement`, `docs`). | [C] Configurable |
| 2 | [PR Size Labeler](#2-pr-size-labeler) | `pr-size-labeler.yml` | Automatically labels PRs based on total lines changed (`size/XS` through `size/XXL`). | [D] Drop-in |
| 3 | [Auto-Label PRs by Path](#3-auto-label-prs-by-path) | `pr-path-labeler.yml` | Applies labels based on which files/directories a PR modifies (e.g., `frontend`, `backend`, `docs`). | [C] Configurable |
| 4 | [Duplicate Issue Detector](#4-duplicate-issue-detector) | `duplicate-detector.yml` | Uses word-overlap similarity to detect potential duplicate issues and comments with links to the originals. | [C] Configurable |

### Quality & Compliance

| # | Workflow | File | Description | Portability |
|---|---------|------|-------------|:-----------:|
| 5 | [PR Issue Assignment Check](#5-pr-issue-assignment-check) | `pr-issue-check.yml` | Ensures every PR references an issue and that the PR author is assigned to it. Closes non-compliant PRs. | [D] Drop-in |
| 6 | [PR Title Linter (Conventional Commits)](#6-pr-title-linter) | `pr-title-lint.yml` | Enforces Conventional Commit format on PR titles (e.g., `feat:`, `fix:`, `docs:`). | [C] Configurable |
| 7 | [WIP / Draft Enforcer](#7-wip--draft-enforcer) | `wip-enforcer.yml` | Fails CI if a PR title contains `[WIP]` or `WIP:` to prevent accidental merging of work-in-progress. | [D] Drop-in |
| 8 | [PR Description Checklist Validator](#8-pr-description-checklist-validator) | `pr-checklist-check.yml` | Ensures all checklist items (`- [ ]`) in the PR template are checked before allowing merge. | [D] Drop-in |
| 9 | [Issue Template Checker](#9-issue-template-checker) | `issue-template-check.yml` | Verifies that new issues actually filled out the issue template instead of leaving it blank or deleting it. | [C] Configurable |

### Lifecycle & Cleanup

| # | Workflow | File | Description | Portability |
|---|---------|------|-------------|:-----------:|
| 10 | [Stale Issue & PR Closer](#10-stale-issue--pr-closer) | `stale-issues.yml` | Labels and closes issues/PRs that have been inactive for a configurable number of days. | [C] Configurable |
| 11 | [Needs Info Timeout](#11-needs-info-timeout) | `needs-info-timeout.yml` | When a `needs-info` label is added, posts a comment. If no reply after 14 days, auto-closes the issue. | [C] Configurable |
| 12 | [Lock Closed Threads](#12-lock-closed-threads) | `lock-threads.yml` | Locks issues and PRs that have been closed for 30+ days to prevent necro-bumping. | [C] Configurable |
| 13 | [Auto-Close Spam Issues](#13-auto-close-spam-issues) | `close-spam-issues.yml` | Detects issues that are likely spam (very short body, suspicious patterns) and closes them with a label. | [D] Drop-in |

### Assignment & Community

| # | Workflow | File | Description | Portability |
|---|---------|------|-------------|:-----------:|
| 14 | [Issue Assigner on Comment](#14-issue-assigner-on-comment) | `issue-assign-on-comment.yml` | Allows contributors to self-assign issues by commenting `/assign`. Optionally limits assignments per user. | [C] Configurable |
| 15 | [Auto-Assign Reviewers](#15-auto-assign-reviewers) | `auto-assign-reviewers.yml` | Automatically assigns reviewers to new PRs using round-robin selection from a configured team. | [S] Needs Setup |
| 16 | [First-Time Contributor Welcome](#16-first-time-contributor-welcome) | `welcome-contributor.yml` | Posts a friendly welcome comment when a user opens their first issue or PR in the repository. | [C] Configurable |

### Security & Quality

| # | Workflow | File | Description | Portability |
|---|---------|------|-------------|:-----------:|
| 17 | [Secret Leak Scanner (Gitleaks)](#17-secret-leak-scanner) | `gitleaks-scan.yml` | Scans every PR for accidentally committed secrets, API keys, and credentials using Gitleaks. | [D] Drop-in |
| 18 | [Dependency Review](#18-dependency-review) | `dependency-review.yml` | Checks PRs for newly added dependencies with known vulnerabilities before they are merged. | [D] Drop-in |
| 19 | [Auto-Merge Dependabot PRs](#19-auto-merge-dependabot-prs) | `dependabot-auto-merge.yml` | Automatically merges Dependabot PRs for patch/minor version bumps after CI passes. | [C] Configurable |
| 20 | [License Compliance Check](#20-license-compliance-check) | `license-check.yml` | Scans dependencies and ensures no incompatible licenses (e.g., GPL in an MIT project) are introduced. | [C] Configurable |
| 21 | [CODEOWNERS Validator](#21-codeowners-validator) | `codeowners-validator.yml` | Validates that the `CODEOWNERS` file references only existing users/teams and has no syntax errors. | [D] Drop-in |

### Release & Changelog

| # | Workflow | File | Description | Portability |
|---|---------|------|-------------|:-----------:|
| 22 | [Release Drafter](#22-release-drafter) | `release-drafter.yml` | Automatically drafts GitHub Releases by categorizing merged PRs by their labels into a changelog. | [C] Configurable |
| 23 | [Auto-Tag on Merge](#23-auto-tag-on-merge) | `auto-tag.yml` | Creates a new Git tag when a version bump is detected in a config file (e.g., `package.json`, `pyproject.toml`). | [C] Configurable |
| 24 | [Publish Release on Tag](#24-publish-release-on-tag) | `publish-release.yml` | Creates a GitHub Release with auto-generated notes whenever a new version tag is pushed. | [D] Drop-in |

### Content Moderation

| # | Workflow | File | Description | Portability |
|---|---------|------|-------------|:-----------:|
| 25 | [AI Content Detector](#25-ai-content-detector) | `ai-content-detector.yml` | Reviews PR/issue descriptions and comments for signs of AI-generated content and posts an advisory. | [C] Configurable |

### Repository Hygiene

| # | Workflow | File | Description | Portability |
|---|---------|------|-------------|:-----------:|
| 26 | [Branch Cleanup](#26-branch-cleanup) | `branch-cleanup.yml` | Deletes branches that have been merged or are stale (no commits for 90+ days). | [C] Configurable |
| 27 | [Old Workflow Run Cleanup](#27-old-workflow-run-cleanup) | `cleanup-workflow-runs.yml` | Periodically deletes old workflow run logs and artifacts to save storage. | [C] Configurable |
| 28 | [Broken Link Checker](#28-broken-link-checker) | `broken-links.yml` | Scans all markdown files in the repository for broken links (404s) on a weekly schedule. | [D] Drop-in |
| 29 | [TODO/FIXME Tracker](#29-todofixme-tracker) | `todo-tracker.yml` | Scans PRs for new `TODO` or `FIXME` comments and posts a summary comment on the PR. | [D] Drop-in |

### Reporting & Notifications

| # | Workflow | File | Description | Portability |
|---|---------|------|-------------|:-----------:|
| 30 | [Weekly Activity Digest](#30-weekly-activity-digest) | `weekly-digest.yml` | Generates a weekly summary of issues opened/closed, PRs merged, and contributors, posted to Slack/Discord. | [S] Needs Setup |
| 31 | [Add Issues/PRs to Project Board](#31-add-issuesprs-to-project-board) | `add-to-project.yml` | Automatically adds new issues and PRs to a specified GitHub Project board. | [S] Needs Setup |
...
- **Portability:** [C] Edit the keyword-to-label mapping to match your label taxonomy.
...
- **Portability:** [D] Works as-is. Change thresholds if desired.
...
- **Portability:** [C] Edit the `PATH_LABELS` mapping in the workflow to match your project structure.
...
- **Portability:** [C] Adjust `SIMILARITY_THRESHOLD` (0.0-1.0) and `MAX_RESULTS`. For better semantic matching, swap in an AI-based approach.
...
- **Portability:** [D] Works as-is for any repository.
...
- **Portability:** [C] Configure allowed types (`feat`, `fix`, `docs`, `chore`, etc.) and whether scope is required.
...
- **What it does:** Fails the CI check if the PR title starts with `[WIP]`, `WIP:`, or `Draft:`. Also detects the construction sign emoji.
- **Manual dispatch:** Lists all open WIP PRs (informational, does not fail).
- **Portability:** [D] Works as-is.
...
- **Portability:** [D] Works for any repo that uses a PR template with checklists.
...
- **Portability:** [C] Adjust minimum body length and required section headers.
...
- **Portability:** [C] Customize `days-before-stale`, `days-before-close`, messages, and exempt labels.
...
- **Portability:** [C] Adjust the label name and timeout period.
...
- **Portability:** [C] Adjust `issue-inactive-days` and lock reason message.
...
- **Portability:** [D] Works as-is. Can optionally tune spam patterns.
...
- **Portability:** [C] Adjust the max assignment limit.
...
- **Portability:** [S] Requires a list of reviewer usernames and possibly a PAT for team-based assignment.
...
- **Portability:** [C] Customize the welcome messages. Optionally link to your `CONTRIBUTING.md`.
...
- **Portability:** [D] Works as-is. Optionally add a `.gitleaks.toml` for custom rules/allowlists.
...
- **Portability:** [D] Works for any repo using npm, pip, Maven, Go modules, Cargo, etc.
...
- **Portability:** [C] Requires "Allow auto-merge" enabled in repo settings. Adjust to restrict to patch-only if desired.
...
- **Portability:** [C] Configure the `deny-licenses` or `allow-licenses` list for your project.
...
- **Portability:** [D] Works for any repo with a `CODEOWNERS` file.
...
- **Portability:** [C] Requires a `.github/release-drafter.yml` config to define label categories and version resolution. Template included in workflow comments.
...
- **Portability:** [C] Checks multiple version files in order; customize the list if needed.
...
- **Portability:** [D] Works as-is for any repo using semver tags.
...
- **Portability:** [C] Adjust `EMOJI_THRESHOLD`, `EM_DASH_THRESHOLD`, `PHRASE_PATTERNS`, and `SCORE_THRESHOLD`.
...
- **Portability:** [C] Adjust `STALE_DAYS` and `PROTECTED_PATTERNS` to match your branching model.
...
- **Portability:** [C] Adjust `RETENTION_DAYS`.
...
- **Portability:** [D] Works as-is. Optionally customize `KEYWORDS`.
...
- **Portability:** [S] Requires a `WEBHOOK_URL` repo secret (Slack or Discord). Set `WEBHOOK_PLATFORM` to `"slack"` or `"discord"`.
...
- **Portability:** [S] Requires the Project URL and a PAT with `project` scope stored as `PROJECT_TOKEN` secret.
...
|:----:|---------|---------------------|
| [D] | **Drop-in** | Copy the file, commit, done. No changes required. |
| [C] | **Configurable** | Works immediately but review the config values (timeouts, labels, thresholds) and adjust to your preferences. |
| [S] | **Needs Setup** | Requires external configuration: API keys, webhook URLs, PATs, Project IDs, or third-party service accounts. |
